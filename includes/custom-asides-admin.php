	...
	?>
		<tr class="form-field">
			<th scope="row" valign="top">
				<label for="custom_asides"><?php _e( 'Выбрать сайдбар', PSTUCTVSTZS_TEXTDOMAIN ); ?></label>
			</th>
			<td>
				<select class="pstuctvstzs-control" id="custom_asides" name="custom_asides">
					<option value=""><?php _e( 'Стандартный сайдбар', PSTUCTVSTZS_TEXTDOMAIN ); ?></option>
...
add_action( 'category_edit_form_fields', 'pstuctvstzs\edit_category_custom_asides' );
...
add_action( 'category_add_form_fields', 'pstuctvstzs\add_category_custom_asides' );
...
add_action( 'create_category', 'pstuctvstzs\save_category_custom_asides' );
add_action( 'edited_category', 'pstuctvstzs\save_category_custom_asides' );
...
function add_custom_asides_meta_box() {
	if ( ! in_array( get_post_meta( get_the_ID(), '_wp_page_template', true ), array( 'singular-fluid.php' ) ) ) {
		add_meta_box(
			PSTUCTVSTZS_SLUG . '_custom_asides',
			__( 'Пользовательские сайдбары', PSTUCTVSTZS_TEXTDOMAIN ),
			'pstuctvstzs\render_custom_asides_meta_box',
			array( 'page' ),
			'side'
		);
	}
}
add_action( 'add_meta_boxes', 'pstuctvstzs\add_custom_asides_meta_box' );
...
function render_custom_asides_meta_box( $post, $meta ) {
	...
	if ( ! empty( $register_asides ) ) : ?>
		<name class="pstuctvstzs-name" for="custom_asides"><?php _e( 'Выбрать сайдбар', PSTUCTVSTZS_TEXTDOMAIN ); ?></name>
		<select class="pstuctvstzs-control" id="custom_asides" name="custom_asides">
			<option value=""><?php _e( 'Стандартный сайдбар', PSTUCTVSTZS_TEXTDOMAIN ); ?></option>
			<?php foreach ( $register_asides as $aside ) : ?>
				<option value="<?php echo $aside[ 'id' ]; ?>" <?php selected( $custom_asides, $aside[ 'id' ], true ); ?> ><?php echo $aside[ 'name' ]; ?></option>
...
add_action( 'save_post', 'pstuctvstzs\save_custom_asides_metadata' );
...
function register_custom_asides_submenu() {
	add_theme_page(
		...
		'manage_options',
		PSTUCTVSTZS_SLUG . '_custom_asides',
		'pstuctvstzs\render_custom_asides_submenu'
	);
}
add_action( 'admin_menu', 'pstuctvstzs\register_custom_asides_submenu' );


function action_for_custom_asides( $current_screen ) {
	if ( 'appearance_page_' . PSTUCTVSTZS_SLUG . '_custom_asides' == $current_screen->id ) {
		if (
			...
			wp_safe_redirect( get_admin_url( null, 'themes.php?page=' . PSTUCTVSTZS_SLUG . '_custom_asides', null ), 302 );
			exit();
		}
	}
}
add_action( 'current_screen', 'pstuctvstzs\action_for_custom_asides', 10, 1 );
...
function render_custom_asides_submenu() {
	...
	$page_url = 'themes.php?page=' . PSTUCTVSTZS_SLUG . '_custom_asides';
	$page_content = ( isset( $_REQUEST[ 'notice' ] ) ) ? $_REQUEST[ 'notice' ] : '';
	$page_title = get_admin_page_title();
	ob_start();
	if ( 'add' == $current_tab || empty( $register_asides ) ) : ?>
		<h3><?php _e( 'Добавление', PSTUCTVSTZS_TEXTDOMAIN ); ?></h3>
		<form method="post">
			...
			<p>
				<input type="text" name="new_aside[name]" required="required" placeholder="<?php esc_attr_e( 'Название', PSTUCTVSTZS_TEXTDOMAIN ); ?>">
				<input type="text" name="new_aside[description]" placeholder="<?php esc_attr_e( 'Описание', PSTUCTVSTZS_TEXTDOMAIN ); ?>">
				<input type="text" name="new_aside[class]" placeholder="<?php esc_attr_e( 'CSS-класс', PSTUCTVSTZS_TEXTDOMAIN ); ?>">
				<button class="button button-primary" type="submit"><?php _e( 'Добавить', PSTUCTVSTZS_TEXTDOMAIN ); ?></button>
				<a class="button" href="<?php echo $page_url; ?>"><?php _e( 'Отмена', PSTUCTVSTZS_TEXTDOMAIN ); ?></a>
			</p>
		</form>
	<?php elseif ( 'edit' == $current_tab && isset( $_REQUEST[ 'id' ] ) ) : ?>
		<h3><?php _e( 'Редактирование', PSTUCTVSTZS_TEXTDOMAIN ); ?></h3>
		<?php
		...
		<form method="post">
			<p>
				...
				<input type="text" name="new_value[name]" required="required" value="<?php echo esc_attr( $old_value[ 'name' ] ); ?>" placeholder="<?php esc_attr_e( 'Название', PSTUCTVSTZS_TEXTDOMAIN ); ?>">
				<input type="text" name="new_value[description]" value="<?php echo esc_attr( $old_value[ 'description' ] ); ?>" placeholder="<?php esc_attr_e( 'Описание', PSTUCTVSTZS_TEXTDOMAIN ); ?>">
				<input type="text" name="new_value[class]" value="<?php echo esc_attr( $old_value[ 'class' ] ); ?>" placeholder="<?php esc_attr_e( 'CSS-класс', PSTUCTVSTZS_TEXTDOMAIN ); ?>">
			</p>
			<p>
				<a class="button" href="<?php echo $page_url; ?>"><?php _e( 'Отмена', PSTUCTVSTZS_TEXTDOMAIN ); ?></a>
				<button class="button button-primary" type="submit"><?php _e( 'Сохранить', PSTUCTVSTZS_TEXTDOMAIN ); ?></button>
			</p>
		</form>
	<?php else : ?>
		<?php
			add_thickbox();
			$count = 0;
			$page_title = $page_title . ' <a class="button button-primary" href="' . $page_url . '&tab=add">' . __( 'Добавить', PSTUCTVSTZS_TEXTDOMAIN ) . '</a>';
			$registered_sidebars = wp_get_sidebars_widgets();
		?>
		<table class="custom-aside-table">
			<thead>
				<th><?php _e( '№', PSTUCTVSTZS_TEXTDOMAIN ); ?></th>
				<th><?php _e( 'Название', PSTUCTVSTZS_TEXTDOMAIN ); ?></th>
				<th><?php _e( 'Описание', PSTUCTVSTZS_TEXTDOMAIN ); ?></th>
				<th><?php _e( 'CSS-класс', PSTUCTVSTZS_TEXTDOMAIN ); ?></th>
				<th><?php _e( 'Страницы', PSTUCTVSTZS_TEXTDOMAIN ); ?></th>
				<th><?php _e( 'Виджеты', PSTUCTVSTZS_TEXTDOMAIN ); ?></th>
			</thead>
			<tbody>
				<?php foreach ( $register_asides as $register_aside ) : ?>
					...
					<tr>
						...
						<td class="posts">
							<div id="usage-<?php echo $register_aside[ 'id' ]; ?>" style="display:none;">
								<?php if ( is_array( $pages ) && ! empty( $pages ) ) : $usage = $usage + count( $pages ); ?>
									<h4><?php _e( 'Страницы', PSTUCTVSTZS_TEXTDOMAIN ); ?></h4>
									<ul class="list-disc">
								...
								<?php else : ?>
									<p><?php _e( 'Колонка не используется на постоянных страницах.', PSTUCTVSTZS_TEXTDOMAIN ); ?></p>
								<?php endif; ?>
								<?php if ( is_array( $categories ) && ! empty( $categories ) ) : $usage = $usage + count( $categories ); ?>
									<h4><?php _e( 'Категории', PSTUCTVSTZS_TEXTDOMAIN ); ?></h4>
									<ul class="list-disc">
								...
								<?php else : ?>
									<p><?php _e( 'Колонка не используется на страницах категорий.', PSTUCTVSTZS_TEXTDOMAIN ); ?></p>
								<?php endif; ?>
							</div>
						...
						<td class="text-right">
							<a class="action-button delete-button" onclick="return confirm( '<?php esc_attr_e( 'Вы уверены?', PSTUCTVSTZS_TEXTDOMAIN ); ?>' );" href="<?php echo $action_url . '&action=delete'; ?>"><?php _e( 'Удалить', PSTUCTVSTZS_TEXTDOMAIN ); ?></a>
							<a class="action-button edit-button" href="<?php echo $action_url . '&tab=edit'; ?>"><?php _e( 'Редактировать', PSTUCTVSTZS_TEXTDOMAIN ); ?></a>
						</td>
					</tr>
				<?php endforeach; ?>
			</tbody>
